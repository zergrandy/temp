...
function processData(data) {
  ...
  if (data.areThereAvailablePromoCodes !== undefined)
    setAvailablePromoCodes(data.areThereAvailablePromoCodes);
...
function customInfoRequest(customInfoRequest) {
  switch (customInfoRequest.input.type) {
    case "numerical":
      $("#custom-screen2-numerical-title").text(
        customInfoRequest.screen2.title
      );
    ...
    case "text":
      ...
      $(".key.backspace.standard-backspace-key").removeClass(
      ...
      customRequirementTextKeyboard.setConstraint(
        customInfoRequest.input.constraintType,
        ["#submit-text-requirement"]
      );
      if (customInfoRequest.input.constraintType === "spaceSeparation") {
        $("#optional-text-field-2").show();
        $(".key.backspace.standard-backspace-key").addClass(
          "backspace-margin-left-override"
        );
      ...
      break;
    case "choiceList":
      $("#custom-screen2-choiceList-title").text(
        customInfoRequest.screen2.title
      );
      $("#custom-screen2-choiceList-text").text(customInfoRequest.screen2.text);
      customRequirementChoiceList.replaceChoices(
        customInfoRequest.input.choiceList,
        customInfoRequest.input.constraintType
      );
...
function chooseCoin(coins, twoWayMode) {
  if (twoWayMode) {
    $(".choose_coin_state")
      .removeClass("choose-coin-cash-in")
      .addClass("choose-coin-two-way");
  } else {
    $(".choose_coin_state")
      .removeClass("choose-coin-two-way")
      .addClass("choose-coin-cash-in");
  }
...
$(document).ready(function () {
  ...
  usSsnKeypad = new Keypad("us-ssn-keypad", { type: "usSsn" }, function (
    result
  ) {
  ...
  phoneKeypad = new Keypad(
    "phone-keypad",
    { type: "phoneNumber", country: "US" },
    function (result) {
  ...
  securityKeypad = new Keypad("security-keypad", { type: "code" }, function (
    result
  ) {
  ...
  customRequirementNumericalKeypad = new Keypad(
    "custom-requirement-numeric-keypad",
    {
      type: "custom",
    },
    function (result) {
...
function setWifiList(recs, requestedPage) {
  ...
  for (var i = 0; i < len; i++) {
    ...
    var html =
      ...
      rec.displaySsid +
      "</span>" +
      '<div class="wifiicon-wrapper"><img src="images/wifiicon/' +
      bars +
      '.svg"/></div></div>';
  ...
  var button =
    '<span display="inline-block" id="more-networks" class="button filled-action-button tl2">' +
    moreTxt +
    "</span>";
  if (recs.length > 4) {
...
function setHardLimit(limits) {
  ...
  if (limits.hardLimitDays >= 1) {
    return component.text(
      translate("Please come back in %s days and %s hours", [
        limits.hardLimitDays,
        limits.hardLimitHours,
      ])
    );
  }

  component.text(
    translate("Please come back in %s hours", [limits.hardLimitHours])
  );
}
...
function setWifiSsid(data) {
  ...
  t(
    "wifi-for-ssid",
    translate("for %s", ["<strong>" + data.ssid + "</strong>"])
  );
  t(
    "wifi-connect",
    translate("You're connecting to the WiFi network %s", [
...
function setChooseCoinColors() {
  ...
  if (isTwoWay) {
    $(".choose_coin_state .change-language")
      .removeClass("cash-in-color")
      .addClass("cash-out-color");
  } else {
    $(".choose_coin_state .change-language")
      .removeClass("cash-out-color")
      .addClass("cash-in-color");
  }
}
...
function setPrimaryLocales(primaryLocales) {
  ...
  closeLanguageDropdown();
  languages.empty();
  var sortedPrimaryLocales = primaryLocales
    .filter(lookupLocaleNames)
    .sort(function (a, b) {
  ...
  $(".js-multi-language").toggleClass(
    "hide",
    sortedPrimaryLocales.length === 2
  );
...
function buildCassetteButtons(_cassettes, numberOfButtons) {
  ...
  for (var i = 0; i < sortedCassettes.length; i++) {
    ...
    $(".cash-button[data-denomination-index=" + i + "] .js-denomination").text(
      denomination
    );
  }
}
...
function updateCassetteButtons(activeDenoms, numberOfButtons) {
  for (var i = 0; i < numberOfButtons; i++) {
    var button = $(
      ".choose_fiat_state .cash-button[data-denomination-index=" + i + "]"
    );
...
function buildCassetteButtonEvents() {
  ...
  touchImmediateEvent(fiatButtons, function (e) {
    ...
    buttonPressed("fiatButton", {
      denomination: cashButtonJ.children(".js-denomination").text(),
    });
  });
}
...
function highBill(highestBill, reason) {
  ...
  t(
    "highest-bill",
    translate("Please insert %s or less.", [formatFiat(highestBill)])
  );
...
function minimumTx(lowestBill) {
  t(
    "lowest-bill",
    translate("Minimum first bill is %s.", [formatFiat(lowestBill)])
  );
...
function readingBill(bill) {
  $(".js-processing-bill").html(
    translate("Processing %s ...", [formatFiat(bill)])
  );
...
function sendOnly(reason) {
  ...
  if (reason === "blockedCustomer") {
    $(".js-send-only-text").text(
      translate(
        "Due to local regulations, you've reached your transaction limit. Please contact us if you'd like to raise your limit."
      )
    );
  } else {
    $(".js-send-only-text").text("");
...
function translateCoin(cryptoCode) {
  $(".js-i18n-scan-your-address").html(
    translate("Scan your <br/> %s address", [cryptoCode])
  );
  $(".js-i18n-please-scan").html(
    translate("Please scan the QR code <br/> to send us your %s.", [cryptoCode])
  );
  $(".js-i18n-did-send-coins").html(
    translate("Have you sent the %s yet?", [cryptoCode])
  );
  $(".js-i18n-scan-address").html(
    translate("Scan your %s address", [cryptoCode])
  );
  $(".js-i18n-invalid-address").html(
    translate("Invalid %s address", [cryptoCode])
  );
}
...
function displayCrypto(cryptoAtoms, cryptoCode) {
  ...
  var cryptoAmount = new BigNumber(cryptoAtoms)
    .div(scale)
    .round(decimalPlaces)
    .toNumber();
...
function fiatCredit(data) {
  ...
  if (cryptoAtoms.eq(0) || cryptoAtoms.isNaN())
    $("#js-i18n-choose-digital-amount").hide();
...
function setDepositAddress(depositInfo) {
  ...
  $(".deposit_state .send-notice .crypto-address").html(
    formatAddress(depositInfo.toAddress)
  );
...
function fiatReceipt(tx) {
  ...
  $(".fiat_receipt_state .sent-coins .crypto-address").html(
...
function fiatComplete(tx) {
  ...
  $(".fiat_complete_state .sent-coins .crypto-address").html(
    formatAddress(tx.toAddress)
  );
...
function setReceiptPrint(receiptStatus, smsReceiptStatus) {
  ...
  var printing = receiptStatus ? "列印收據中..." : "收據簡訊發送中...";
  var success = receiptStatus ? "收據列印成功!" : "收據簡訊成功發送!";
